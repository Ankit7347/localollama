...
def process_syllabus_file(input_file):
    output_dir = "data/output"
    os.makedirs(output_dir, exist_ok=True)
    filename = os.path.basename(input_file)
    output_file = os.path.join(output_dir, filename.replace(".json", "-topic.json"))
...
def main():
    data_files = [f for f in glob.glob("data/official_syllabus_*.json") if not f.endswith("-topic.json")]
    for f in data_files:
        process_syllabus_file(f)
